...
public:
	...
	int isIntersecting(bool* t_visited,bool* s_visited);

	void addEdge(int u,int v);

	void print_path(int* s_parent,int* t_parent,int s,int t,int intersectNode);

	void BFS(list<int>* queue,bool* visited,int* parent);

	int birDirSearch(int s,int t);
...
void Graph::print_path(int* s_parent,int* t_parent,int s,int t,int intersectNode){
...
int Graph:: birDirSearch(int s,int t){
	...
	while(!s_queue.empty() && !t_queue.empty()){
		...
		if(intersectNode!=-1){
            ...
            print_path(s_parent,t_parent,s,t,intersectNode);
